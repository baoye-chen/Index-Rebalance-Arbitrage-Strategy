...
class Russell3000:
    ...
    def book_beta(self, date, current_index):
        ...
        for stock in list(self.book.keys()):
            ...
            price = prices[prices['Date'] == date]['Open']
            k = 1
            while len(price) == 0:
                price = prices[prices['Date'] == self.account.index[current_index - k]]['Open']
                k += 1
            price = float(price)
    ...
    def backtest(self, start_y, start_m, start_d, num_of_days, verbose=False,
                 stop_loss=False, store_in_bank=False, beta_hedge=False):
        ...
        exit_dates = self.signal['close_date'].unique()
        rank_dates = self.signal['dates'].unique()
        ...
        for j, i in enumerate(self.account.index):
            ...
            for stock in list(self.book):
                prices = pd.read_csv(self.price_data_path + stock + '.csv')
                price = prices[prices['Date'] == i]['Open']
                k = 1
                while len(price) == 0:
                    price = prices[prices['Date'] == self.account.index[j - k]]['Open']
                    k += 1
                price = float(price)
            ...
            if beta_hedge and len(self.book) != 0:
                etf_prices = pd.read_csv(self.price_data_path + "IWV.csv")
                etf_price = etf_prices[etf_prices['Date'] == i]['Open']
                k = 1
                while len(etf_price) == 0:
                    etf_price = etf_prices[etf_prices['Date'] == self.account.index[j - k]]['Open']
                    k += 1
                self.hedge_notional = self.hedge_position * float(etf_price)

            if i in rank_dates:
                stocks = self.signal[self.signal['dates'] == i]
                ...
                for stock in long.index:
                    self.need_to_trade[stock] = [long_notional, long.loc[stock]['enter_date'],
                                                 long.loc[stock]['close_date'], long.loc[stock]['beta']]
                ...
                for stock in short.index:
                    self.need_to_trade[stock] = [short_notional, short.loc[stock]['enter_date'],
                                                 short.loc[stock]['close_date'], short.loc[stock]['beta']]

                if verbose:
                    print("a rank date. Signal Generated.")
                    print("need to trade")
            ...
            if i in enter_dates:
                ...
                for stock in list(self.need_to_trade):
                    if self.need_to_trade[stock][1] == i:
                        prices = pd.read_csv(self.price_data_path + stock + '.csv')
                        price = float(prices[prices['Date'] == i]['Open'])
                ...
                if beta_hedge:
                    ...
                    etf_prices = pd.read_csv(self.price_data_path + "IWV.csv")
                    etf_price = float(etf_prices[etf_prices['Date'] == i]['Open'])
                ...
                if verbose:
                    ...
                    print(self.book)
                    if beta_hedge:
                        print("total beta of today's book = " + str(self.notionals *
                                                                    self.book_beta(date=i, current_index=j)))
                        print("hedging " + str(self.hedge_position) + " shares of IWV to neutralize beta")
